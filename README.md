...
| 模块 | 能力 |
| --- | --- |
| Repository | 导入 PDF 文件夹和 BibTeX 文件，解析论文文本并写入 SQLite，支持导入任务状态和真实库统计 |
...
| Citation | 根据查询生成 `\cite{}`、`\citep{}`、`\citet{}`；多候选时可在控制台中选择真实 BibTeX key |
...
- 查询命中多个引用候选时，控制台会要求选择其中一条，再生成并复制 LaTeX 引用命令。
...
| v0.2 | 进行中：导入任务状态、真实库统计、FTS5 检索、本地 LLM provider 修正、source snippets 展示、引用候选选择；后续补 OCR、文献去重、引用候选排序 |
